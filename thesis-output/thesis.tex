...
\newcommand{\thesisdate}{August 16, 2021}
...
\caption{Development and forecast of the mean full-load-hours. Figure taken from Deutsche WindGuard 2020}\label{fig:flh}
\end{figure}
...
\end{figure}
With a fixed nominal output, a high full-load hourly value is also associated with a high energy yield. For example, the full load hours achieved by a wind energy installation in different years can be compared well with one another in order to make a statement about the electricity yield for a year. However, If two plants have the same hub height and the same rotor diameter, but a different rated capacity, the less powerful plant, i.e., the plant with the lower specific rated capacity, will achieve more full-load-hours but a lower annual energy yield at the same wind speeds. The full-load-hourly value should therefore always be considered and evaluated in connection with the energy yield. The development of the rotor diameter and hub height is shown in Figure \ref{fig:rotor} and \ref{fig:nabe}.
\begin{figure}
...
\end{figure}
Hence, the methodology of the DWG is adopted and applied in an own analysis of the electricity yield, the rated capacity and the full-load-hours as well as rotor diameter and hub height in RLP and the results are presented in section 4.1.
...
In order to calculate the electricity yield of future plants in RLP, the following section analyses the master and movement data from 2019 for electricity yield, nominal output and full-load-hours. Additionally the area consumption of WTs in RLP is being assessed using the market master data register.
